...
/**
 * Class IDEATest This test performs IDEA encryption then decryption. IDEA stands for International
 * Data Encryption Algorithm. The test is based on code presented in Applied Cryptography by Bruce
 * Schnier, which was based on code developed by Xuejia Lai and James L. Massey.
 **/
...
class IDEATest {
    ...
    /* buildTestData Builds the data used for the test -- each time the test is run. */
    ...
    /* calcEncryptKey Builds the 52 16-bit encryption subkeys Z[] from the user key and stores in
     * 32-bit int array. The routing corrects an error in the source code in the Schnier book.
     * Basically, the sense of the 7- and 9-bit shifts are reversed. It still works reversed, but
     * would encrypted code would not decrypt with someone else's IDEA code. */
    ...
    /* calcDecryptKey Builds the 52 16-bit encryption subkeys DK[] from the encryption- subkeys Z[].
     * DK[] is a 32-bit int array holding 16-bit values as unsigned. */
    ...
    /* mul Performs multiplication, modulo (2**16)+1. This code is structured on the assumption that
     ...
     * mod 0x10001). NOTE: The java conversion of this routine works correctly, but is half the
     * speed of using Java's modulus division function (%) on the multiplication with a 16-bit
     * masking of the result--running in the Symantec Caje IDE. So it's not called for now; the test
     * uses Java % instead. */
    ...
    /* inv Compute multiplicative inverse of x, modulo (2**16)+1 using extended Euclid's GCD
     * (greatest common divisor) algorithm. It is unrolled twice to avoid swapping the meaning of
     * the registers. And some subtracts are changed to adds. Java: Though it uses signed 32-bit
     * ints, the interpretation of the bits within is strictly unsigned 16-bit. */
    ...
    /* freeTestData Nulls arrays and forces garbage collection to free up memory. */
...
class IDEARunner implements Runnable {
    ...
    /* run() IDEA encryption/decryption algorithm. It processes plaintext in 64-bit blocks, one at a
     * time, breaking the block into four 16-bit unsigned subblocks. It goes through eight rounds of
